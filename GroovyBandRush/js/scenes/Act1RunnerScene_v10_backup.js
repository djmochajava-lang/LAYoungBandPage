...
var Act1RunnerScene = new Phaser.Class({
  ...
  drawBackground: function () {
    ...
    // Vibrant city skyline — buildings on SIDES of road only
    ...
    // Generate buildings for LEFT side (0 to roadLeft)
    var buildings = [];
    var bx = 0;
    while (bx < roadL - 4) {
      var bw = Phaser.Math.Between(18, 38);
      var bh = Phaser.Math.Between(50, 160);
      // Clamp building to not spill into road
      if (bx + bw > roadL) bw = Math.max(8, roadL - bx);
      buildings.push({ x: bx, w: bw, h: bh });
      bx += bw + Phaser.Math.Between(1, 4);
    }

    // Generate buildings for RIGHT side (roadRight to width)
    bx = roadR;
    while (bx < width + 10) {
      var bw2 = Phaser.Math.Between(18, 38);
      var bh2 = Phaser.Math.Between(50, 160);
      buildings.push({ x: bx, w: bw2, h: bh2 });
      bx += bw2 + Phaser.Math.Between(0, 3);
    }

    // Building silhouettes — extend all the way down to screen bottom
    for (var b = 0; b < buildings.length; b++) {
      ...
      skyline.fillRect(bld.x, skylineY - bld.h, bld.w - 2, bld.h + (height - skylineY) + 20);
    }

    // Colorful lit windows — above skyline (sky-facing portion) AND below (street-level storefronts)
    var windowColors = [0xffd700, 0x00e5ff, 0xe84393, 0xffffff, 0xe8751a, 0x3498db];
    for (var bwi = 0; bwi < buildings.length; bwi++) {
      var bl = buildings[bwi];
      // Windows above skyline (upper building faces)
      ...
      // Street-level windows (lower floors alongside road, sparser)
      for (var wy2 = skylineY + 10; wy2 < height - 20; wy2 += 18) {
        for (var wx2 = bl.x + 3; wx2 < bl.x + bl.w - 5; wx2 += 9) {
          if (Math.random() > 0.55) {
            var wc2 = Phaser.Math.RND.pick(windowColors);
            skyline.fillStyle(wc2, Phaser.Math.FloatBetween(0.04, 0.18));
            skyline.fillRect(wx2, wy2, 3, 6);
          }
        }
      }
    }
  ...
  drawRoad: function () {
    ...
    // Sidewalk / curb strip between road edges and buildings
    var sideBg = this.add.graphics();
    sideBg.setDepth(1);
    // Narrow sidewalk strips right next to road
    sideBg.fillStyle(0x222230, 0.8);
    sideBg.fillRect(this.roadLeft - 6, height * 0.3, 6, height * 0.7);
    sideBg.fillRect(this.roadRight, height * 0.3, 6, height * 0.7);
    // Side neon strips (glow from road edge onto buildings)
    sideBg.fillStyle(0xe8751a, 0.06);
    sideBg.fillRect(this.roadLeft - 10, height * 0.3, 10, height * 0.7);
    sideBg.fillRect(this.roadRight, height * 0.3, 10, height * 0.7);
  ...
  spawnObstacle: function () {
    ...
    // Scale to fit lane — same proportion as player car
    obstacle.setDisplaySize(this.laneWidth * 0.55, this.laneWidth * 0.85);

    // Headlight glow effect (warm cone in front of oncoming car)
    var headlight = this.add.graphics().setDepth(7);
    headlight.fillStyle(0xffffaa, 0.12);
    headlight.fillEllipse(x, -60 + 50, 36, 14);
